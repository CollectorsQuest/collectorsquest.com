...
<?php
	...
	$title		 	= $wpdb->get_var($wpdb->prepare("SELECT `name` FROM `".$wpdb->prefix."adrotate_blocks` WHERE `id` = '%s';", $block_edit_id));
	$stats 			= $wpdb->get_row($wpdb->prepare("SELECT SUM(`clicks`) as `clicks`, SUM(`impressions`) as `impressions` FROM `".$wpdb->prefix."adrotate_stats_tracker` WHERE `block` = '%s';", $block_edit_id));
	$stats_today 	= $wpdb->get_row($wpdb->prepare("SELECT `clicks`, `impressions` FROM `".$wpdb->prefix."adrotate_stats_tracker` WHERE `block` = '%s' AND `thetime` = '$today';", $block_edit_id));
...
<table class="widefat" style="margin-top: .5em">
  	...
  	<tr>
        <th colspan="5">
			...
			if($blockstats) {
				...
				echo "<img src=\"".plugins_url("adrotate/library/graph_block.php?title=$impressions_title&data=$impressions_array", "AdRotate")."\" />";

				$clicks_title = urlencode(serialize(__('Clicks over the past 21 days', 'adrotate')));
				$clicks_array = urlencode(serialize($clicks_array));
				echo "<img src=\"".plugins_url("adrotate/library/graph_block.php?title=$clicks_title&data=$clicks_array", "AdRotate")."\" />";
			} else {
    ...
    <tr>
		...
		<td width="40%" colspan="4">
			<?php wp_nonce_field('adrotate_report_blocks','adrotate_nonce'); ?>
	    	<input type="hidden" name="adrotate_export_id" value="<?php echo $block_edit_id; ?>" />
			<input type="hidden" name="adrotate_export_type" value="block" />
