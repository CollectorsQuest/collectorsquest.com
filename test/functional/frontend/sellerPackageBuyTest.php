...
$browser->
    ...
    with('response')->begin()->
      ...
      checkElement('form *[type="submit"]', true)->
    end();
...
$browser->
    ...
    with('form')->begin()->
      ...
      setFormField($formName, 'promo_code', 'CQ2012-FIX-20')->
      click('applyPromo3')->
    end()->
...
$browser->
    ...
    with('form')->begin()->
      ...
      setFormField($formName, 'fyi', true)->
...
$browser->
    ...
    get('/seller/packages')->
    click('Purchase Package', array(
      'packages'=> array(
        'package_id'  => 1,
        'payment_type'=> 'cc',
        'cc_type'     => 'Visa',
        'cc_number'   => '4296642165025622', # a test CC
        'expiry_date' => array(
          'month'=> 01,
          'year' => 2017,
        ),
