...
class _ajaxActions extends cqFrontendActions
{
  ...
  public function executeFeedback(sfWebRequest $request)
  {
  	...
  	{
  		$this->form->setDefault('fullname', $this->getUser()->getUsername());
  		$this->form->setDefault('email', $this->getUser()->getEmail());
  	}
  	...
  	if ($request->isMethod('post'))
  	{
  		...
  		if ($sent)
  		{
  			$this->getUser()->setFlash('success_ajax', $this->__('Thank you for the feedback. If needed, we will get in touch with you within the next business day.', array(), 'flash'));
  		}
  		else
  		{
